      ...
      <section className="mt-8 grid gap-6 xl:grid-cols-[1fr_360px]">
        <Card>
          ...
          {cockpit.runs.length ? (
            ...
            <div className="rounded-3xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center text-sm text-slate-500">Noch keine lokalen Runs gefunden. Starte den Generator, um ein lokales Review-Paket zu erzeugen.</div>
          )}
        </Card>

        <Card className="relative overflow-hidden bg-slate-950 text-white shadow-[0_28px_90px_rgba(15,23,42,0.24)]">
          ...
          <div className="relative">
            <p className="text-sm font-bold uppercase tracking-[0.18em] text-white/60">MVP Flow</p>
            <h2 className="mt-3 text-3xl font-semibold tracking-[-0.05em]">Vom Rohtext zum Review-Paket</h2>
            <div className="mt-6 grid gap-3 text-sm leading-6 text-white/78">
              ...
              <p>3. Artefakte für JSON, Markdown, CSV, XLSX und ZIP bereitstellen.</p>
            </div>
            <Link href="/testfall-automation/library" className="mt-7 inline-flex rounded-2xl bg-white/92 px-5 py-3 text-sm font-bold text-slate-950 shadow-[0_18px_40px_rgba(255,255,255,0.12)] transition hover:bg-white">Bibliothek öffnen</Link>
          </div>
        </Card>
      </section>
    </TestForgeShell>
...
function StatCard({ label, value, detail, tone }: { label: string; value: string | number; detail: string; tone: "violet" | "emerald" | "rose" | "amber" }) {
  const colors = {
    violet: "bg-slate-950 shadow-slate-200",
...
function RunRow({ run }: { run: TestfallRunSummary }) {
  return (
    <Link href={`/testfall-automation/runs/${run.runId}`} className="grid gap-4 rounded-3xl border border-white/70 bg-white/55 p-4 shadow-sm backdrop-blur-xl transition hover:-translate-y-0.5 hover:border-slate-300 hover:bg-white/80 hover:shadow-[0_18px_50px_rgba(15,23,42,0.08)] md:grid-cols-[1fr_auto] md:items-center">
      ...
      <div className="text-left text-sm text-slate-400 md:text-right">
        ...
        <p className="mt-1 font-semibold text-slate-800">Details öffnen →</p>
      </div>
    </Link>
  );
}
